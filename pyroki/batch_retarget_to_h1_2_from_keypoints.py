...
def get_humanoid_retarget_indices() -> jnp.ndarray:
    ...
    h1_2_joint_retarget_indices = []

    # NOTE: the order matters here.
    for human_name, h1_2_name in [
        ("pelvis", "pelvis"),  # source animation key point, target robot link
        ...
        h1_2_joint_retarget_indices.append(H1_2_LINK_NAMES.index(h1_2_name))

    h1_2_joint_retarget_indices = jnp.array(h1_2_joint_retarget_indices)

    return human_retarget_names, h1_2_joint_retarget_indices
...
def load_motion_data(
    ...
    if source_type == "smpl":
        ...
        simplified_keypoints_root = (
            simplified_keypoints_root * onp.array([1.1, 1.1, 1.1])[None, :]
        )
        ...
        simplified_keypoints_lower_body_local = (
            simplified_keypoints_lower_body_local
            * onp.array([1.0, 1.0, 1.0])[None, None, :]
        )
        ...
        simplified_keypoints_upper_body_local = (
            simplified_keypoints_upper_body_local
            * onp.array([1.0, 1.0, 0.9])[None, None, :]
        )
        ...
        simplified_keypoints_root = (
            simplified_keypoints_root * onp.array([1.0, 1.0, 1.0])[None, :]
        )
        simplified_keypoints = (
            simplified_keypoints_root[:, None, :] + simplified_keypoints_local
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=str(
            SCRIPT_DIR / "../protomotions/data/assets/urdf/for_retargeting/h1_2.urdf"
        ),
    ...
    parser.add_argument(
        "--mesh-dir",
        type=str,
        default=str(SCRIPT_DIR / "../protomotions/data/assets/mesh/H1_2"),
    ...
    if H1_2_LINK_NAMES is None:
        H1_2_LINK_NAMES = list(robot.links.names)

    global human_retarget_names, h1_2_joint_retarget_indices
    human_retarget_names, h1_2_joint_retarget_indices = get_humanoid_retarget_indices()

    current_motion_index = 0

    # Create connectivity matrix for the H1_2 joints
    n_retarget = len(h1_2_joint_retarget_indices)
    h1_2_retarget_mask = jnp.zeros((n_retarget, n_retarget))
    for link_a, link_b, weight in direct_pairs:
        ...
        h1_2_retarget_mask = h1_2_retarget_mask.at[retarget_idx_a, retarget_idx_b].set(
            weight
        )
        h1_2_retarget_mask = h1_2_retarget_mask.at[retarget_idx_b, retarget_idx_a].set(
            weight
        )
    ...
    if args.visualize:
        ...
        def generate_trajectory():
            ...
            Ts_world_root, joints = solve_retargeting(
                ...
                h1_2_joint_retarget_indices=h1_2_joint_retarget_indices,
...
def foot_contact_cost(
    ...
    h1_2_joint_retarget_indices: jnp.ndarray,
    ...
    left_ankle_robot_idx = h1_2_joint_retarget_indices[left_ankle_idx]
    right_ankle_robot_idx = h1_2_joint_retarget_indices[right_ankle_idx]
    left_foot_robot_idx = h1_2_joint_retarget_indices[left_foot_idx]
    right_foot_robot_idx = h1_2_joint_retarget_indices[right_foot_idx]
...
def solve_retargeting(
    ...
    # Variables.
    class SimplifiedJointsScaleVarH1_2(
    ...
    var_joints_scale = SimplifiedJointsScaleVarH1_2(jnp.zeros(timesteps))
    ...
    def retargeting_cost(
        ...
        var_Ts_world_root: jaxls.SE3Var,
        var_robot_cfg: jaxls.Var[jnp.ndarray],
        var_joints_scale: SimplifiedJointsScaleVarH1_2,
        ...
        robot_pos = T_world_link.translation()[jnp.array(h1_2_joint_retarget_indices)]
    ...
    def pc_alignment_cost(
        ...
        T_root_link = jaxlie.SE3(robot.forward_kinematics(cfg=robot_cfg))
        T_world_link = T_world_root @ T_root_link
        link_pos = T_world_link.translation()[h1_2_joint_retarget_indices]

        # hand_aux_link 1
        left_wrist = human_retarget_names.index("left_wrist")
        left_wrist_idx = h1_2_joint_retarget_indices[left_wrist]
        ...
        right_wrist_idx = h1_2_joint_retarget_indices[right_wrist]
        ...
        right_hand_aux_pos = (
            link_pos_right_wrist + link_rot_mat_right_wrist @ jnp.array([0.0, 0.0, 0.2])
        )

        # torso aux link

        # TODO: different from G1, no transform from pelvis to torso.
        torso_idx = H1_2_LINK_NAMES.index("torso_link")
    ...
    costs = [
        ...
        #     jnp.full(var_joints.default_factory().shape, 0.02) # small rest cost for all joints
        #     .at[joints_to_move_less]
        #     .set(weights["joint_rest_penalty"])[None], # large rest cost for joints that should move less
        # ),
