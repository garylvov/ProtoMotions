...
"""BeyondMimic + L2C2 + 0.64 s lookahead ladder (H1-2 augmented-corpus recipe).

Copy of mlp_bm_l2c2.py with ONE change: future_steps [1,2,4,8] -> [1,2,4,8,16,32].

Why this file exists. The 0.64 s ladder landed in mimic/mlp.py, but mlp.py uses the
CLASSIC mimic tracking rewards (mimic_tracking_rewards_factory + pow/contact_match),
not BeyondMimic. mlp_bm_l2c2.py has the BeyondMimic reward set but was left on the
0.16 s ladder. So as shipped you could have the long horizon OR the BM rewards, not
both. Training the pickup/squat corpus wants both: BM's region-weighted relative-body
terms, and a horizon long enough to see a squat coming.

NOT in this file, deliberately -- the auxiliary rewards (wrist_dir, root_gain,
hold_balance) are runtime env-var opt-ins read in envs/base_env/hold_fix.py:195-243,
all defaulting to 0.0. They are set at launch, not in an experiment config, so this
config does not and cannot enable them. See the launch command in
docs/h1_2-augmented-training.md.
...
def env_config(robot_cfg: RobotConfig, args: argparse.Namespace) -> EnvConfig:
    ...
    control_components = {
        "mimic": MimicControlConfig(
            ...
            # Lookahead ladder, matching mimic/mlp.py. env dt = decimation/fps
            # = 4/200 = 0.02 s for h1_2 on isaaclab (robot_configs/h1_2.py), so
            # this spans 0.02 .. 0.64 s where the parent config spanned only
            # 0.02 .. 0.16 s. A 0.16 s horizon cannot anticipate a footfall or a
            # weight transfer -- a step is ~0.5-0.6 s -- which is the whole point
            # of the pickup/squat corpus this config is meant to train on.
            # Dense near-term for tracking accuracy, sparse far-term for intent.
            #
            # NOTE this widens mimic_target_poses ~1.5x vs the parent (6 future
            # blocks instead of 4), so checkpoints from mlp_bm_l2c2 will NOT
            # load against this config. From-scratch retrain, not a warmstart.
